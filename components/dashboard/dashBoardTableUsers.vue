  ...
  <v-card>
    ...
    >
      ...
      <template #[`item.fullName`]="{item}">
        <div class="d-flex align-center">
          ...
          <div class="d-flex flex-column ms-3">
            ...
            <small class="accent--text">{{ item.userName }}</small>
          </div>
        </div>
      </template>
      ...
      <template #[`item.role`]="{item}">
        <div class="d-flex align-center">
          ...
          >
            <v-icon
              size="20"
              :color="`${role[item.role].color}--text`"
            >
              {{ role[item.role].icon }}
            </v-icon>
          </v-avatar>
      ...
      <template #[`item.status`]="{item}">
        <v-chip
          small
          :color="`${statusColor[item.status]} darken-4`"
          class="white--text v-chip-light-bg font-weight-semibold text-capitalize"
        >
          {{ item.status }}
        </v-chip>
      </template>
...
export default {
    data: () => ({
        ...
        role: {
            maintainer: { icon: 'mdi-chart-timeline-variant', color: 'primary darken-4' },
            subscriber: { icon: 'mdi-account-outline', color: 'warning darken-4' },
            editor: { icon: 'mdi-pencil-outline', color: 'info darken-4' },
            author: { icon: 'mdi-cog-outline', color: 'success darken-4' },
        },
        userListData: [
            {
                avatar: '1.png',
